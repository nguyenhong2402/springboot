...
public class EmployerController {
    ...
    public String edittEmployer(@Valid @ModelAttribute("employerForm") EmployerRequest employerRequest,@PathVariable("id") String employerId,
            BindingResult result, Locale locale, Model model) {
        if (employerRequest.getLogo().getOriginalFilename().isEmpty()) {
            result.addError(new FieldError("employerForm", "logo", messageSource.getMessage("employer.logo.notblank", null, locale)));
        }
        ...
        employerService.edit(employerRequest ,employerId);
        return "redirect:/employer";
    }
}
